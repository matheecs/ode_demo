  ...
  if (n > 0) {
    for (i = 0; i < n; i++) {
      ...
      dJointAttach(c, dGeomGetBody(contact[i].geom.g1),
                   dGeomGetBody(contact[i].geom.g2));  //�W���C���g����������
    }
  }
}
  ...
  for (i = 0; i < bodyCount; i++) {
    ...
    switch (body[i]->k) {
      case 'b':
        ...
        sides[1] = body[i]->w;
        sides[2] = body[i]->h;
        dsDrawBox(dBodyGetPosition(body[i]->b), dBodyGetRotation(body[i]->b),
                  sides);  //���`�\��
        break;
      case 's':
        dsDrawSphere(dBodyGetPosition(body[i]->b), dBodyGetRotation(body[i]->b),
                     body[i]->r);  //���`�\��
        break;
      case 'c':
        dsDrawCapsule(dBodyGetPosition(body[i]->b),
                      dBodyGetRotation(body[i]->b), body[i]->l,
                      body[i]->r);  //�J�v�Z���`�\��
        break;
      case 'y':
        dsDrawCylinder(dBodyGetPosition(body[i]->b),
                       dBodyGetRotation(body[i]->b), body[i]->l,
                       body[i]->r);  //�~���`�\��
        break;
      default:
...
static void setJoint(jointStr *j, char k, bodyStr *b1, bodyStr *b2, char a,
  ...
  switch (k) {
    case 'h':  //�q���W�W���C���g
      ...
      switch (a) {                          //����ݒ�
        case 'x':
          dJointSetHingeAxis(j->j, 1, 0, 0);
          break;  // X���̐ݒ�
        case 'z':
          dJointSetHingeAxis(j->j, 0, 0, 1);
          break;  // Z���̐ݒ�
        default:
          dJointSetHingeAxis(j->j, 0, 1, 0);
          break;  // Y���̐ݒ�
      }
      break;
...
static void createBody() {
  ...
  setBody(&K1_r, 'y', 'w', 34, 0, 0, 12, 0, -fw, 195, 0,
          0.05);                            //�Ҋ֐߃��[��
  ...
  setBody(&A1_r, 'y', 'w', 34, 0, 0, 12, 0, -fw, 15, 0,
          0.02);                            //���񃍁[��
  dRFromAxisAndAngle(R, 0, 1, 0, -M_PI_2);  //��]
  ...
  setJoint(&K1J_r, 'h', &DOU, &K1_r, 'x', 0, -fw + 11,
           195);  //�Ҋ֐߃��[��
  ...
  setJoint(&A1J_r, 'h', &A1_r, &sole_r, 'x', 0, -fw + 11,
           15);  //���񃍁[��
...
void destroyBot() {
  ...
  for (i = 0; i < jointCount; i++) {
    if (joint[i]->e > 0) {
      dJointDestroy(joint[i]->j);
    }  //�W���C���g�j��
  }
  for (i = 0; i < bodyCount; i++) {
    if (body[i]->e > 0) {
      dBodyDestroy(body[i]->b);
    }  //�{�f�B�L���Ȃ�j��
    if (body[i]->ge > 0) {
      dGeomDestroy(body[i]->g);
    }  //�W�I���g���ݒ肳��Ă���j��
  }
