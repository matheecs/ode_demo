...
dBodyID link[NUM];    // Linkslink[0] is a base
dJointID joint[NUM];  // Joints    joint[0] is a fixed joint between a base and
                      // a ground
...
void control() {        /***  P control  ****/
  ...
  double k1 = 10.0,
         fMax = 100.0;  // k1: proportional gain,  fMax：Max torque[Nm]
  ...
  for (int j = 1; j < NUM; j++) {
    ...
    dJointSetHingeParam(joint[j], dParamVel,
                        k1 * z);  // Set angular velocity[m/s]
...
void start() {                        /*** Initialize drawing API ***/
  ...
  float hpr[3] = {-160.0, 4.50,
                  0.00};     // View direction(heading, pitch, roll)[°]
...
void command(int cmd) { /*** Keyboard function ***/
  switch (cmd) {
    case 'j':
      THETA[1] += 0.05;
      break;  // When j key is pressed, THETA[1] is increases at 0.05[rad]
    case 'f':
      THETA[1] -= 0.05;
      break;
    case 'd':
...
void simLoop(int pause) {
  ...
  for (int i = 0; i < NUM; i++)  // Draw capsules for links
    dsDrawCapsuleD(dBodyGetPosition(link[i]), dBodyGetRotation(link[i]), l[i],
                   r[i]);
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < NUM; i++) {
    ...
    dMassSetCapsuleTotal(&mass, m[i], 3, r[i],
                         l[i]);    // Calculate mass parameter
    dBodySetMass(link[i], &mass);  // Set mass
  }

  joint[0] = dJointCreateFixed(world, 0);  // A fixed joint
  dJointAttach(joint[0], link[0],
               0);           // Attach the joint between the ground and the base
